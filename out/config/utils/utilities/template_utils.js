...
class TemplateUtils {
    ...
    static interfaceClassImplementsTemplate(featureName, type) {
        ...
        const folder = `${(type === interface_class_type_1.EnumInterfaceClassName.Datasource) ? 'datasources' : 'repositories'}`;
        const fileExtension = `${(type === interface_class_type_1.EnumInterfaceClassName.Datasource) ? 'datasource' : 'repository'}.dart`;
        const fullClass = `${className}${(type === interface_class_type_1.EnumInterfaceClassName.Datasource) ? 'Datasource' : 'Repository'}`;
    ...
    static classFileName(featureName, type, isImplementation) {
        const extension = () => {
            switch (type) {
                case interface_class_type_1.EnumInterfaceClassName.Datasource:
                    return (isImplementation) ? 'datasource_impl' : 'datasource';
                case interface_class_type_1.EnumInterfaceClassName.Repository:
