...
export async function infrastructureRepositories(path: string, featureName: string) {
    ...
    fs.writeFileSync(dirPath.join(fullPath, TemplateUtils.classFileName(featureName, EnumInterfaceClassName.Repository, false)), TemplateUtils.interfaceClassImplementsTemplate(featureName, EnumInterfaceClassName.Repository));
}