...
export async function infrastructureDatasources(path: string, featureName: string) {
    ...
    fs.writeFileSync(dirPath.join(fullPath, TemplateUtils.classFileName(featureName, EnumInterfaceClassName.Datasource, false)), TemplateUtils.interfaceClassImplementsTemplate(featureName,EnumInterfaceClassName.Datasource));
}
